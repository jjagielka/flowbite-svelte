<script lang="ts">
  ...
  const copyToClipboard = async (e: MouseEvent) => {
    ...
    const decodedText = code.innerText.replace(REG_HEX, function (_match: string, group1: string) {
    ...
    <div class="relative border-gray-200 border-y border-x code-syntax dark:border-gray-600">
      ...
      <div class="relative">
        <div class="overflow-hidden" class:max-h-72={!expand} tabindex="-1" use:checkOverflow>
          <div class="highlight">
            <pre bind:this={code} class="language-svelte -mt-2! rounded-none!">{@render code?.()}</pre>
          </div>
        </div>
